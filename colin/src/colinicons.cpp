...
const QIcon &colinIcons::icon(const wgv_load::form &f, bool tooltip) const
{
    if(tooltip)
    {
        switch(f)
...
const QIcon &colinIcons::icon(const wgv_bearing::bearings &b) const
{
    Colin::Actions a = Colin::drawBearing;
    if((b&wgv_bearing::X)==wgv_bearing::X)      a |= Colin::drawBearingH;
    if((b&wgv_bearing::Z)==wgv_bearing::Z)      a |= Colin::drawBearingV;
    if((b&wgv_bearing::Phi)==wgv_bearing::Phi)  a |= Colin::drawBearingM;
    return icon(a);
}
