...
void MainWindow::setTw(wgv_tw *tw)
{
    if(tw == NULL)
    {
...
void MainWindow::launchToolTip()
{
    ...
    p = mapToGlobal(toolbar->actionGeometry(senderA).bottomLeft())+QPoint(0, 24);
...
void MainWindow::calcFinished()
{
    ...
    p = mapToGlobal(toolbar->actionGeometry(calcA).bottomLeft())+QPoint(0, 24);
...
void MainWindow::paste()
{
    ...
    {
        if(tw->load(0).typ() == wgv_load::nodeLoad ||
           tw->load(0).typ() == wgv_load::moment   )
...
{
    structPrinter printer(filelist::instance().currentFile());
...
{
    wgv_tw *t = filelist::instance().currentFile();
    for(int i=0; i<4; i++)
    {
        t->view(i).scale(1.4,
                         1.4);
