						...
						const QTransform &t, int *adi, bool ignoreOff)
{
	...
	{
	    if(tw.load(m).typ()==ColinLoad::nodeLoad)
	    {
		dp = tw.node(tw.load(m).at()).toQPointF()
		     -QPointF(tw.load(m).Px(), tw.load(m).Pz())*tw.scaleP();
	    }
	    else if(tw.load(m).typ()==ColinLoad::decreasingLinearLoad)
	    {
		dp = tw.load(m).shape().at(1);
	    }
	    else if(tw.load(m).typ()==ColinLoad::uniformlyDistibutedLoad)
	    {
		dp = tw.load(m).shape().at(1)/2+tw.load(m).shape().at(2)/2;
	    }
	    else if(tw.load(m).typ()==ColinLoad::increasingLinearLoad)
	    {
	...
	if((((*c & CatchedMLine) == CatchedMLine) ||
		((*c & CatchedQLine) == CatchedQLine) ||
		((*c & CatchedNLine) == CatchedNLine) ||
		((*c & CatchedULine) == CatchedULine))&&
		tw.isCalculated())
	{

		for(int i=0; i<tw.beam_n(); i++){
			double x = tw.nearestPointOnBeam(t.inverted().map(*mouse), i);
			if(x<0 || x>tw.beam(i).l())
				continue;
			QTransform local_t;
			local_t.translate(tw.beam(i).leftNode().x(), tw.beam(i).leftNode().z());
			local_t.rotate(tw.beam(i).angle()*180.0/M_PI);
			if((*c & CatchedMLine) == CatchedMLine)
			{
				foreach(int ls, tw.activeCLS())
				{
			...
			if((*c & CatchedULine) == CatchedULine)
			{
				foreach(int ls, tw.activeCLS())
				{
					...
					do{
						x_wa=x_w;
						x_w = x-tw.beam(i).u(ls, x_w)*tw.scaleU();
					}while(x_wa-x_w>0.01);
