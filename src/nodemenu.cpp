    ...
    ColinMenu()
{
    ...
    g->addButton(h, ColinSupport::X);
    g->addButton(v, ColinSupport::Z);
    g->addButton(m, ColinSupport::Phi);
...
{
    ColinSupport::bearings form = ColinSupport::NoBear;
    foreach(QAbstractButton *b, g->buttons())
    {
        if(b->isChecked())
            form |= static_cast<ColinSupport::bearing>(g->id(b));
    }


    if(form == ColinSupport::NoBear)
        actions()[6]->setIcon(QIcon());
    else
        actions()[6]->setIcon(colinIcons::instance().icon(ColinSupport::formToDrawEnum(form)));
    ...
    b.setX(form.testFlag(ColinSupport::X));
    b.setZ(form.testFlag(ColinSupport::Z));
    b.setPhi(form.testFlag(ColinSupport::Phi));
