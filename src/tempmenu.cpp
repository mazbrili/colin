    ...
    ColinMenu()
{
    ...
    h = new ColinPushButtonPart(colinIcons::instance().icon(ColinLoad::tempChange),"");
    v = new ColinPushButtonPart(colinIcons::instance().icon(ColinLoad::tempDiffrence),"");

    g = new QButtonGroup(this);
    g->setExclusive(true);
    g->addButton(h, ColinLoad::tempChange);
    g->addButton(v, ColinLoad::tempDiffrence);
...
void tempMenu::set(const int &i)
{
    ...
    if(filelist::instance().currentFile()->load(i).typ() == ColinLoad::tempChange)
    {
        h->setChecked(true);
...
{
    ColinLoad l(filelist::instance().currentFile()->load(cTemp));
...
void tempMenu::tempChanged()
{
    ...
    if(ok)
    {
        if(filelist::instance().currentFile()->load(cTemp).typ() == ColinLoad::tempChange)
